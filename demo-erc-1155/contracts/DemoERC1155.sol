...
contract DemoERC1155 is ERC1155, Ownable, Pausable, ERC1155Supply, PaymentSplitter {
    ...
    // edit functions
    function editMintWindows(bool _publicMintOpen, bool _allowListOpen) external onlyOwner {
    ...
    function allowListMint(uint256 id, uint256 amount) public payable {
        require(allowListMintOpen, "allowListMint not open");
        require(allowList[msg.sender], "You aer not on allowList");
        require(msg.value == allowListPrice * amount);
        mint(id, amount);
    }

    function publicMint(uint256 id, uint256 amount) public payable {
        require(publicMintOpen, "publicMint not open");
